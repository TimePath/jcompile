...
enum class QInstruction {
    ...
    STOREP_VEC {
        override fun stringify(it: Statement): Array<Any> = arrayOf("*", it.b, "=", it.a)
        override fun action(it: Statement, f: FloatBuffer, i: IntBuffer, s: StringManager, e: EntityManager) {
            val vec = arrayOf(f[it.a + 0], f[it.a + 1], f[it.a + 2])
    ...
    IF {
        override fun stringify(it: Statement): Array<Any> = arrayOf("if", it.a, "then jmp rel", it.b)
        override fun advance(it: Statement, f: FloatBuffer, i: IntBuffer): Int = (if (!!f[it.a]) it.b else 1)
    },
    IFNOT {
        override fun stringify(it: Statement): Array<Any> = arrayOf("if not", it.a, "then jmp rel", it.b)
        override fun advance(it: Statement, f: FloatBuffer, i: IntBuffer): Int = (if (!f[it.a]) it.b else 1)
    },

    CALL0 {
        override fun stringify(it: Statement): Array<Any> = arrayOf(it.a, "()")
    },
    CALL1 {
        override fun stringify(it: Statement): Array<Any> = arrayOf(it.a, "(.)")
    },
    ...
    STATE {
        override fun stringify(it: Statement): Array<Any> = arrayOf("self.nextthink", "=", "time + 0.1", ";",
                "self.frame", "=", it.a, ";", // f[it.a]
                "self.think", "=", it.b) // i[it.b]
    },

    GOTO {
        override fun stringify(it: Statement): Array<Any> = arrayOf("jmp", "rel", it.a)
    ...
    protected open fun stringify(it: Statement): Array<Any> = arrayOf("ILLEGAL")
    protected open fun action(it: Statement, f: FloatBuffer, i: IntBuffer, s: StringManager, e: EntityManager): Unit = Unit
